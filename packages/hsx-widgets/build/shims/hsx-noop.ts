...
function ssrOnly(name: string): never {
  throw new Error(
    `${name}() is SSR-only and cannot be called in embed context`,
  );
}

// Core exports
export function render(): never {
  return ssrOnly("render");
}
export function renderHtml(): never {
  return ssrOnly("renderHtml");
}
export function route(): never {
  return ssrOnly("route");
}
export function id(): never {
  return ssrOnly("id");
}
...
// Component exports
export function hsxComponent(): never {
  return ssrOnly("hsxComponent");
}
export function hsxPage(): never {
  return ssrOnly("hsxPage");
}
...
export type Route<_P extends string, _Params = unknown> = {
  path: _P;
  build: (params: _Params) => string;
};
export type HsxComponent<
  _P extends string,
  _Params = unknown,
  _Props = unknown,
> = unknown;
